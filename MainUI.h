...
int SaveAndExit(sf::RenderWindow* Oldwindow, std::vector<Creature*> creatures)
{
	...
	for (size_t i = 0; i < creatures.size(); i++)
	{
		...
		f.write(reinterpret_cast<const char*>((&Temp)), sizeof(int)); // ends 1
...
int ContinueSimulation(sf::RenderWindow* Oldwindow)
{
	...
	//Firstly just create it by the default directory
	std::string SavePath = "Saves/Save.txt";
	std::string SaveLocatorData;

	std::ifstream SaveLocator(SavePath);
	getline(SaveLocator, SaveLocatorData);

	SaveLocator.close();

	std::vector<std::string> FileNameData = explode(SaveLocatorData, *",");
	...
	int NodeNum;
	float weight;
	Position TempLoc = Position(0,0);

	for (size_t n = 0; n < NumberofCreatures; n++)
	{

		CreatureData->Creatures.push_back(new Creature);
		std::ifstream File("Saves\\"+(FileNameData[n] + ".CSD"), std::ios::in | std::ios::binary);

		File.read(reinterpret_cast<char*>(&MuscleNum), sizeof(int));
		std::cout << MuscleNum << std::endl;
		for (size_t i = 0; i < MuscleNum; i++)
		{
			CreatureData->Creatures.back()->Muscles.push_back(new Muscle);
			File.read(reinterpret_cast<char*>(&fre), sizeof(float));
			File.read(reinterpret_cast<char*>(&str), sizeof(float));
			File.read(reinterpret_cast<char*>(&ends1), sizeof(int));
			File.read(reinterpret_cast<char*>(&ends2), sizeof(int));
			File.read(reinterpret_cast<char*>(&biases), sizeof(float[4]));
			...
			CreatureData->Creatures.back()->Muscles.back()->Strength = str;
			CreatureData->Creatures.back()->Muscles.back()->Frequency = fre;
		...
		for (size_t i = 0; i < NodeNum; i++)
		{
			...
			File.read(reinterpret_cast<char*>(&TempLoc), sizeof(Position));

			CreatureData->Creatures.back()->Nodes.back()->Weight = weight;
			CreatureData->Creatures.back()->Nodes.back()->Location = TempLoc;
			std::cout << CreatureData->Creatures.back()->Nodes.back()->Location.x << CreatureData->Creatures.back()->Nodes.back()->Location.y << "aa" << std::endl;
		}
		for (size_t i = 0; i < MuscleNum; i++)
		{
			CreatureData->Creatures.back()->Muscles[i]->ends[0] = CreatureData->Creatures.back()->Nodes[ends1];
			CreatureData->Creatures.back()->Muscles[i]->ends[1] = CreatureData->Creatures.back()->Nodes[ends2];
		}
